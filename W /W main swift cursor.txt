...
struct MLPrediction {
    ...
    var timestamp: Date  // ← CHANGE: from 'let' to 'var'
    let model: String
}
    ...
    static func updateDashboardJSON() {
        Task { @MainActor in
            ...
            do {
                ...
                print("✅ Updated dashboard.json with live data: \(livePrices.count) pairs, \(totalTrades) trades")
                logAlert(event: "✅ Dashboard updated: \(livePrices.count) pairs, $\(String(format: "%.2f", totalProfit)) profit")
...
""")
    ...
    static func startComprehensiveTradingSystem() {
        ...
        logAlert(event: "🚀 Comprehensive AI trading system started with all components")
    }
}
...
extension ExchangeWebSocket {
    ...
    static func executeTrade(exchange: String, pair: String, side: String, price: Double, amount: Double, strategy: String) async -> Bool {
        ...
        // In simulation, allow passed amount even if balance is zero; otherwise cap by dynamic sizing
        let adjustedAmount: Double
        if simulationEnabled {
            adjustedAmount = max(amount, 0.0)
        } else {
            guard dynamicAmount > 0 else {
                print("DEBUG: Trade skipped for \(exchange)-\(pair): Invalid trade amount (\(dynamicAmount))")
                return false
            }
            adjustedAmount = min(amount, dynamicAmount)
        }
...
extension ExchangeWebSocket {
    ...
    // NEW: SINGLE CONSISTENT CSV LOGGING FUNCTION
    // Format: timestamp,exchange,pair,side,price,amount,profit,fees,type (9 fields ALWAYS)
    static func logConsistentTrade(exchange: String, pair: String, side: String, price: Double, amount: Double, profit: Double, fees: Double, type: String) async {
        ...
        await MainActor.run {
            ...
            performanceMetrics[pair]!.profit += (profit - fees)
            performanceMetrics[pair]!.fees += fees
            if (profit - fees) > 0 {
                performanceMetrics[pair]!.wins += 1
            }
            
            // FIXED: ALWAYS 9 fields in this exact order
            let tradeEntry = "\(timestamp),\(exchange),\(pair),\(side),\(String(format: "%.4f", price)),\(String(format: "%.6f", amount)),\(String(format: "%.4f", profit)),\(String(format: "%.4f", fees)),\(type)\n"
            
            do {
                let fileManager = FileManager.default
                // Create directory if needed
                let outputDir = tradeCSVURL.deletingLastPathComponent()
                ...
                logAlert(event: "💰 TRADE LOGGED: \(side) \(pair) on \(exchange) - Profit: $\(String(format: "%.2f", profit - fees))")
            } catch {
